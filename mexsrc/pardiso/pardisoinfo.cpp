...
void reportPardisoInitError (int error) {
    switch(error){
    case -10:
        mexErrMsgTxt("No license file pardiso.lic found");
    case -11:
        mexErrMsgTxt("Pardiso license is expired!");
    case -12:
        mexErrMsgTxt("Wrong username or hostname");
otherwise:{
      char str[256];
      sprintf(str,"PARDISOINIT reports an unknown error of type %d", error);
      mexErrMsgTxt(str);
          }
    }
}
